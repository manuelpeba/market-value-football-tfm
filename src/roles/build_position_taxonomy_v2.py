"""
TM.6.4.2 — Position Taxonomy Calibration
...
Outputs
-------
- data/processed/player_position_taxonomy_v2.parquet
- reports/roles/position_taxonomy_distribution_v2.csv
- reports/roles/position_taxonomy_validation_v2.csv
- reports/roles/position_taxonomy_representatives_v2.csv
- reports/roles/position_taxonomy_purity.csv
...
def calibrate_taxonomy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ATT functional scores
    df["_score_CF"] = (
        0.30 * df["_s_xg"]
        + 0.22 * df["_s_shots"]
        + 0.18 * df["_s_shots_on_target"]
        + 0.20 * df["_s_touches_box"]
        + 0.10 * (1 - df["_s_progressive_receipts"])
    )
    df["_score_W"] = (
        0.24 * df["_s_progressive_receipts"]
        + 0.22 * df["_s_progressive_carries"]
        + 0.18 * df["_s_carries_box"]
        + 0.18 * df["_s_creation_index_role"]
        + 0.10 * df["_s_touches_att_third"]
        + 0.08 * df["_s_key_passes"]
    )
    ...
    df["_score_AM"] = (
        0.34 * df["_s_creation_index_role"]
        + 0.18 * df["_s_touches_att_third"]
        + 0.13 * df["_s_touches_box"]
        + 0.15 * df["_s_key_passes"]
        + 0.12 * df["_s_sca"]
        + 0.08 * df["_s_gca"]
    )
...
def main() -> None:
    ...
    distribution(report_df).to_csv(reports / "position_taxonomy_distribution_v2.csv", index=False)
    validation_report(report_df).to_csv(reports / "position_taxonomy_validation_v2.csv", index=False)
    representatives(report_df).to_csv(reports / "position_taxonomy_representatives_v2.csv", index=False)
    compute_purity(report_df).to_csv(reports / "position_taxonomy_purity.csv", index=False)

    summary = {
        "rows": int(len(output)),
        "distribution": distribution(report_df).to_dict(orient="records"),
        "outputs": [
            str(processed / "player_position_taxonomy_v2.parquet"),
            str(reports / "position_taxonomy_distribution_v2.csv"),
            str(reports / "position_taxonomy_validation_v2.csv"),
            str(reports / "position_taxonomy_representatives_v2.csv"),
            str(reports / "position_taxonomy_purity.csv"),
        ],
    }
